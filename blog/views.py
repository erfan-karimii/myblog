...
def blogs(request):
    if request.GET and not request.GET.get('page'):
        ...
        categorys = Category.objects.all()
        paginator = Paginator(blogs, 4) # Show 2 contacts per page.
...
def ContactUs(request):
    ...
    return render(request,'blog_folder/contact.html',context)    
...
def category_blog_view(request,cat):
    ...
    return render(request,'blog_folder/blog.html',{'blogs':blog})
